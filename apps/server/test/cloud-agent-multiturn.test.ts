...
import { eventsToMessages } from "../src/services/cloud-agent/messages.js";
...
describe("cloud agent multi-turn tool availability", () => {
  it("preserves namespaces in legacy assistant_message events", () => {
    const toolCall = { id: "legacy", type: "function" as const, namespace: "workspace_files", function: { name: "re_fs_stat", arguments: '{"path":"/a.txt"}' } };
    const messages = eventsToMessages([
      { type: "assistant_message", payload: { content: "Inspecting", toolCalls: [toolCall] } },
      { type: "tool_call_result", payload: { toolCallId: toolCall.id, resultText: "File exists" } },
      { type: "user_message", payload: { content: "Read it" } },
    ]);
    assert.deepEqual(messages[0]!.toolCalls, [toolCall]);
    assert.equal((mapMessagesToResponsesInput(messages).input[0] as any).namespace, "workspace_files");
  });

...
授权码、设备码和 refresh token 均只存哈希；凭证接口禁止缓存。设备码不进入浏览器 URL；授权码本身不能领取凭证。相同设备码只能消费一次，并发 refresh 只有一次成功。刷新后旧 WS 以关闭码 `1012` 关闭（不是鉴权失败），客户端按普通断线重连并以新凭据发送 `hello`。`0057` 保留已有设备和未过期的授权请求。
...
每条用户消息限 4000 字符，JSON 帧限 1 MB；5 秒内未完成握手会以 `4408` 关闭（客户端可在取得 token 后重连）。无效、过期或已撤销的凭据使用关闭码 `4401`，无授权绑定使用 `4403`，协议错误使用 `1008`，这三种伴随 `error {fatal:true}`，客户端不应自动重连；凭据轮换使用 `1012`，输出缓冲溢出使用 `1013`，均可重连。连接期间也会重新检查权限和有效期。
